...
# Green: Filmography controls (sidebar)
st.sidebar.header("Filmography options")
search = st.sidebar.text_input("Search title")
sort_by = st.sidebar.selectbox("Sort by", ["Year", "Rating", "Title"], index=1)
ascending = st.sidebar.checkbox("Ascending", value=False)
page_size = st.sidebar.slider("Cards per page", 5, 20, 10, step=5)
...
# Group by integer year and rename for plotting
counts = (
    filtered.groupby('Year_int')
    .size()
...
# Green: Line chart with tooltips + accent color
year_chart = (
    alt.Chart(counts)
    ...
    .encode(
        ...
        tooltip=[alt.Tooltip('Year:Q', format='.0f'), alt.Tooltip('Count:Q', format='d')],
        color=alt.value('#FFC107')  # AlloCiné-ish accent
    )
...
exclusions = [
    "Arcadian","Dream Scenario","Longlegs","The Retirement Plan","Wild at Heart",
    "Ghost Rider: Spirit of Vengeance","The Surfer","Lords of War","Running with the Devil",
    "Jiu Jitsu","The Gunslingers","The Carpenter's Son"
]

# Start from filtered set and apply exclusions
grid = filtered[~filtered['Title'].isin(exclusions)].copy()

# Apply search
if search:
    grid = grid[grid['Title'].str.contains(search, case=False, na=False)]

# Apply sorting
sort_col = {'Year': 'Year_int', 'Rating': 'Rating', 'Title': 'Title'}[sort_by]
grid = grid.sort_values(sort_col, ascending=ascending)

# Pagination
total_items = len(grid)
total_pages = max(1, (total_items + page_size - 1) // page_size)
page = min(page, total_pages)  # clamp page within range
start = (page - 1) * page_size
end = start + page_size
page_df = grid.iloc[start:end]
st.caption(f"Page {page} of {total_pages} — {total_items} movies")

# Render poster grid
data_for_grid = page_df[['Title','Year_int','Rating','Poster','Genre']]
cols_per_row = 5
rows = [data_for_grid.iloc[i:i+cols_per_row] for i in range(0, len(data_for_grid), cols_per_row)]
for row in rows:
